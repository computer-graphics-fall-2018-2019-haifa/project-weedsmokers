	...
	activeModelIndex(0)
{
	...
	/*parallelLight.ambientCoefficient = 0.5;
	parallelLight.attenuation = 1;
	parallelLight.color= glm::vec3(0.4, 0.3, 0.1);
	parallelLight.direction= glm::vec3(1, 0.8, 0.6);*/
	this->parallelLights.push_back(parallelLight);


	PointLight pointLight;
	/*pointLight.position = glm::vec4(-4, 0, 10, 1);
	pointLight.color = glm::vec3(2, 2, 2); //strong white light
	pointLight.attenuation = 0.1f;
	pointLight.coneAngle = 15.0f;
...
/*void Scene::addLight(glm::vec4 position, glm::vec3 color, float attenuation, float ambientCoefficient, float coneAngle, glm::vec3 coneDirection)
{
	Light light;
